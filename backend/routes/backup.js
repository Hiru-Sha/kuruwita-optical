...
router.get('/', auth, async (req, res) => {
  try {
    const [
      ...
      historicalRecords, storeOrders, storeProducts,
      promoCodesRes, storeReviews,
    ] = await Promise.all([
      ...
      // ── New tables added 2026 ────────────────────────────────
      pool.query(`SELECT * FROM historical_records ORDER BY id`).catch(() => ({ rows: [] })),
      pool.query(`SELECT * FROM store_orders ORDER BY id`).catch(() => ({ rows: [] })),
      pool.query(`SELECT * FROM store_products ORDER BY id`).catch(() => ({ rows: [] })),
      pool.query(`SELECT * FROM promo_codes ORDER BY id`).catch(() => ({ rows: [] })),
      pool.query(`SELECT * FROM store_reviews ORDER BY id`).catch(() => ({ rows: [] })),
    ]);
    ...
    res.json({
      ...
      tables: {
        ...
        warranty_claims:   warrantyClaims.rows,
        walkin_rx:         walkinRx.rows,
        historical_records: historicalRecords.rows,
        store_orders:      storeOrders.rows,
        store_products:    storeProducts.rows,
        promo_codes:       promoCodesRes.rows,
        store_reviews:     storeReviews.rows,
      },
      counts: {
        ...
        historical_records: historicalRecords.rows.length,
        store_orders:       storeOrders.rows.length,
        store_products:     storeProducts.rows.length,
        promo_codes:        promoCodesRes.rows.length,
        store_reviews:      storeReviews.rows.length,
      },
    });
  } catch (err) {
