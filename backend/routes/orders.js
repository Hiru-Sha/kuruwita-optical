...
//  Updated: frame/lens buy+sell prices, frame_material
// ============================================================
...
router.post('/', auth, async (req, res) => {
  ...
  try {
    ...
    if (r_sph || l_sph || r_cyl || l_cyl) {
      ...
      `, [orderId, customer_id, r_sph||null,r_cyl||null,r_axis||null,r_add||null,r_va||null,r_pd||null, l_sph||null,l_cyl||null,l_axis||null,l_add||null,l_va||null,l_pd||null, ref_notes||null]);
    }
...
router.patch('/:id', auth, async (req, res) => {
  ...
  try {
    const result = await pool.query(`UPDATE orders SET ${fields.join(', ')} WHERE id = $${values.length} RETURNING *`, values);
