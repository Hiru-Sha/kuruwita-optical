...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .page { width: 148mm; min-height: 210mm; position: relative; padding-bottom: 20mm; }
  .header { background: #0f1f3d; padding: 10mm 12mm 7mm; display: flex; justify-content: space-between; align-items: flex-start; }
  .logo-img { height: 34px; object-fit: contain; }
  .shop-name { color: white; font-size: 13px; font-weight: 700; }
  .shop-sub { color: #c9a84c; font-size: 8px; margin-top: 1px; letter-spacing: 1px; }
  .shop-addr { color: rgba(255,255,255,0.6); font-size: 8px; margin-top: 3px; line-height: 1.5; }
  .gold-bar { height: 3px; background: linear-gradient(90deg,#c9a84c,#f0d080,#c9a84c); }
  .status-strip { padding: 5px 12mm; font-size: 9px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase; }
  .body { padding: 6mm 12mm; }
  .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 5mm; margin-bottom: 5mm; }
  .info-box { border: 1px solid #e8eaf0; border-radius: 7px; overflow: hidden; }
  .info-head { background: #f5f7ff; padding: 4px 9px; font-size: 7px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase; color: #6b7280; border-bottom: 1px solid #e8eaf0; }
  .info-row { display: flex; justify-content: space-between; padding: 4px 9px; border-bottom: 1px solid #f3f4f6; font-size: 9px; }
  .info-row:last-child { border-bottom: none; }
  .lbl { color: #9ca3af; }
  .val { font-weight: 700; color: #0f1f3d; text-align: right; max-width: 55%; }
  .val.green { color: #166534; }
  .pay-box { border: 1px solid #e8eaf0; border-radius: 7px; overflow: hidden; margin-bottom: 5mm; }
  .pay-head { background: #f5f7ff; padding: 4px 9px; font-size: 7px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase; color: #6b7280; border-bottom: 1px solid #e8eaf0; }
  .pay-row { display: flex; justify-content: space-between; padding: 5px 9px; border-bottom: 1px solid #f3f4f6; font-size: 9px; }
  .pay-row:last-child { border-bottom: none; }
  .pay-row.free { background: #f0fdf4; }
  .pay-row.disc { background: #fefce8; }
  .pay-subtotal { display: flex; justify-content: space-between; padding: 5px 9px; background: #f5f7ff; font-size: 9px; color: #6b7280; border-bottom: 1px solid #e8eaf0; }
  .pay-total { display: flex; justify-content: space-between; padding: 7px 9px; background: #f0f4f8; font-size: 10px; font-weight: 700; border-bottom: 2px solid #0f1f3d; }
  .pay-hl { display: flex; justify-content: space-between; align-items: center; padding: 8px 9px; }
  .pay-bal { display: flex; justify-content: space-between; padding: 6px 9px; background: #fef2f2; }
  .note { border-radius: 6px; padding: 6px 9px; font-size: 8px; line-height: 1.5; margin-bottom: 4mm; }
  .footer { position: absolute; bottom: 0; left: 0; right: 0; border-top: 1px solid #e8eaf0; padding: 4px 12mm; display: flex; justify-content: space-between; }
  .footer span { font-size: 7px; color: #9ca3af; }
...
<div class="page">
  ...
  <div class="header">
    ...
    <div style="text-align:right;">
      <div style="color:#c9a84c;font-size:8px;font-weight:700;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px;">${isAdvance ? 'ADVANCE RECEIPT' : 'FINAL RECEIPT'}</div>
      <div style="background:#c9a84c;color:#0f1f3d;font-weight:700;font-size:13px;padding:3px 10px;border-radius:4px;margin-bottom:3px;">${order.order_number}</div>
      <div style="font-size:7px;color:rgba(255,255,255,0.6);">Date: ${orderDate}</div>
    </div>
  </div>

  <div class="gold-bar"></div>
  <div class="status-strip" style="background:${isAdvance?'#1e3a5f':'#14532d'};color:${isAdvance?'#93c5fd':'#86efac'};">
  ...
  <div class="body">
    ...
    <div class="info-grid">
      <div class="info-box">
        ...
        <div class="info-row"><span class="lbl">Phone</span><span class="val">${order.phone||'—'}</span></div>
        ${order.age ? `<div class="info-row"><span class="lbl">Age</span><span class="val">${order.age} yrs</span></div>` : ''}
      </div>
      <div class="info-box">
        <div class="info-head">Order Details</div>
        <div class="info-row"><span class="lbl">Frame</span><span class="val">${order.frame||'—'}</span></div>
        <div class="info-row"><span class="lbl">Lens</span><span class="val">${order.lens_type||'—'}</span></div>
        <div class="info-row"><span class="lbl">Coating</span><span class="val">${order.lens_coating||'—'}</span></div>
        <div class="info-row"><span class="lbl">Delivery</span><span class="val green">${fdate(order.deliver_date)}</span></div>
      </div>
    </div>

    <div class="pay-box">
      <div class="pay-head">Payment Summary</div>
      ${frameSell > 0 ? `<div class="pay-row"><span class="lbl">Frame (${order.frame_type||''})</span><span style="font-weight:600;">${fmt(frameSell)}</span></div>` : ''}
      ${lensSell  > 0 ? `<div class="pay-row"><span class="lbl">Lens — ${order.lens_type||''} ${order.lens_coating||''}</span><span style="font-weight:600;">${fmt(lensSell)}</span></div>` : ''}
      ${freeItems.length > 0 ? freeItems.map(fi=>`<div class="pay-row free"><span class="lbl" style="color:#166534;">🎁 ${fi.name} ×${fi.qty}</span><span style="font-weight:700;color:#166534;">FREE</span></div>`).join('') : ''}
      ${(discAmt > 0 || discPct > 0) ? `<div class="pay-subtotal"><span>Subtotal</span><span>${fmt(subTotal)}</span></div>` : ''}
      ${discPct > 0 ? `<div class="pay-row disc"><span class="lbl" style="color:#854d0e;">Discount ${discPct}%</span><span style="font-weight:700;color:#c0392b;">- ${fmt(subTotal*discPct/100)}</span></div>` : ''}
      ${discAmt > 0 && discPct === 0 ? `<div class="pay-row disc"><span class="lbl" style="color:#854d0e;">Discount</span><span style="font-weight:700;color:#c0392b;">- ${fmt(discAmt)}</span></div>` : ''}
      <div class="pay-total"><span>Total Amount</span><span>${fmt(total)}</span></div>
      <div class="pay-hl" style="background:${isAdvance?'#dbeafe':'#dcfce7'};">
        <span style="font-size:10px;font-weight:700;color:${isAdvance?'#1e40af':'#166534'};">${isAdvance?'Advance Paid':'Balance Paid'}</span>
        <span style="font-size:14px;font-weight:800;color:${isAdvance?'#1e40af':'#166534'};">${fmt(amountPaid)}</span>
      </div>
      ${remaining > 0 ? `<div class="pay-bal"><span style="font-size:9px;font-weight:700;color:#c0392b;">Balance Remaining</span><span style="font-size:12px;font-weight:800;color:#c0392b;">${fmt(remaining)}</span></div>` : ''}
      ${!isAdvance && remaining <= 0 ? `<div style="text-align:center;padding:6px;background:#f0fdf4;font-weight:700;color:#166534;font-size:9px;">Fully Paid — Balance: Rs. 0.00</div>` : ''}
    </div>

    <div class="note" style="background:${isAdvance?'#fefce8':'#f0fdf4'};border:1px solid ${isAdvance?'#fde68a':'#bbf7d0'};color:${isAdvance?'#92400e':'#166534'};">
      ${isAdvance
        ? `Please bring this receipt when collecting your spectacles. Balance of <b>${fmt(remaining)}</b> is due on collection.`
