...
<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:5px;padding-bottom:4px;border-bottom:2.5px solid #0f1f3d;">
  ...
  <div style="text-align:right;">
    <div style="background:${isAdvance?'#1e40af':'#166534'};color:white;font-weight:700;font-size:8px;padding:2px 8px;border-radius:3px;margin-bottom:2px;text-transform:uppercase;letter-spacing:0.5px;">
      ${isAdvance ? '📋 Advance Receipt' : '✅ Final Receipt'}
    </div>
    <div style="background:#c9a84c;color:#0f1f3d;font-weight:700;font-size:11px;padding:2px 8px;border-radius:4px;margin-bottom:2px;">${order.order_number}</div>
    <div style="font-size:7.5px;color:#6b7280;">Date: ${orderDate}</div>
  </div>
</div>

<!-- CUSTOMER + ORDER in 2 cols -->
<div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;margin-bottom:4px;">
  <div class="section">
    ...
    <div class="row"><span class="lbl">Phone</span><span class="val">${order.phone||'—'}</span></div>
    ${order.age ? `<div class="row"><span class="lbl">Age</span><span class="val">${order.age} yrs</span></div>` : ''}
  </div>
  <div class="section">
    <div class="sec-head">Order Details</div>
    <div class="row"><span class="lbl">Frame</span><span class="val" style="text-align:right;max-width:70px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">${order.frame||'—'}</span></div>
    <div class="row"><span class="lbl">Lens</span><span class="val">${order.lens_type||'—'}</span></div>
    <div class="row"><span class="lbl">Coating</span><span class="val">${order.lens_coating||'—'}</span></div>
    <div class="row"><span class="lbl">Delivery</span><span class="val" style="color:#166534;">${fdate(order.deliver_date)}</span></div>
  </div>
</div>
...
<div class="section" style="margin-bottom:4px;">
  ...
  ${frameSell > 0 ? `<div class="row"><span class="lbl">🕶️ Frame</span><span class="val">${fmt(frameSell)}</span></div>` : ''}
  ${lensSell  > 0 ? `<div class="row"><span class="lbl">🔬 Lens (${order.lens_type||''})</span><span class="val">${fmt(lensSell)}</span></div>` : ''}
  <div class="row" style="background:#f0f4f8;">
    <span style="font-size:10px;font-weight:700;">Total Amount</span>
    <span style="font-size:12px;font-weight:700;color:#0f1f3d;">${fmt(total)}</span>
  </div>
  <div class="row" style="background:${isAdvance?'#dbeafe':'#dcfce7'};">
    <span style="font-weight:700;color:${isAdvance?'#1e40af':'#166534'};">
      ${isAdvance ? '✅ Advance Paid' : '✅ Balance Paid'}
    </span>
    <span style="font-size:13px;font-weight:700;color:${isAdvance?'#1e40af':'#166534'};">${fmt(amountPaid)}</span>
  </div>
  ${remaining > 0 ? `
  <div class="row" style="background:#fef2f2;">
    <span style="font-weight:700;color:#c0392b;">⏳ Balance Due</span>
    <span style="font-size:13px;font-weight:700;color:#c0392b;">${fmt(remaining)}</span>
  </div>` : ''}
  ${remaining <= 0 && !isAdvance ? `
  <div style="text-align:center;padding:4px;background:#dcfce7;font-weight:700;color:#166534;font-size:10px;">
    ✅ Fully Paid — Thank You!
  </div>` : ''}
</div>

<!-- FOOTER -->
<div style="display:flex;justify-content:space-between;align-items:center;border-top:1px solid #d0d7e0;padding-top:3px;">
  <div>
    <div style="font-size:8px;font-weight:700;color:#0f1f3d;">Wickramakalutota Opticals · No.57 Kurunegala Road, Chilaw · 032 222 1211</div>
    ${isAdvance ? '<div style="font-size:7px;color:#c0392b;margin-top:1px;">⚠️ Please bring this receipt when collecting your spectacles.</div>' : '<div style="font-size:7px;color:#166534;margin-top:1px;">🙏 Thank you for trusting Wickramakalutota Opticals!</div>'}
  </div>
