    ...
    <div style={{ display:'flex', minHeight:'100vh', fontFamily:"'Inter','DM Sans',sans-serif", background:'var(--bg)', color:'var(--text)' }}>
      ...
      <>
        ...
        <aside style={{
          ...
          transform: mob ? (open ? 'translateX(0)' : 'translateX(-100%)') : 'translateX(0)',
      ...
      {/* ── Main area ── */}
      <div style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0, marginLeft: mob ? 0 : 240, transition:'margin-left .22s ease', width: mob ? '100%' : 'calc(100% - 240px)' }}>
        ...
        <main style={{
          flex: 1,
          padding: mob ? '14px 12px 90px' : '24px 28px',
          minWidth: 0,
          overflowX: 'hidden',
        }}>
          <Outlet/>
      ...
      {mob && (
        <nav style={{
          ...
          <button onClick={() => setOpen(o => !o)}
            style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:3, background:'none', border:'none', borderTop:`2px solid ${open ? '#c9a84c' : 'transparent'}`, color: open ? '#0f1f3d' : '#9ca3af', fontSize:10, cursor:'pointer', padding:'10px 0' }}>
            <Icon name={open ? 'close' : 'menu'} size={19} color={open ? '#0f1f3d' : '#9ca3af'}/>
            {open ? 'Close' : 'Menu'}
          </button>
        </nav>
      )}
