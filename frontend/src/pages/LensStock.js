...
export default function LensStock() {
  ...
  const [activeTab,  setActiveTab]  = useState('stock'); // stock | add
    ...
    <div style={{ fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:20 }}>
        {[
          ...
          <div key={s.label} style={{ background:s.bg, border:`1.5px solid ${s.bg}`, borderRadius:12, padding:'14px 16px' }}>
          ...
          <div style={{ background:C.cream, borderRadius:10, padding:'14px 16px', marginBottom:16 }}>
            ...
            {form.is_single_side ? (
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
                {[['Right (R)','sph_r','cyl_r'],['Left (L)','sph_l','cyl_l']].map(([label,sphKey,cylKey])=>(
                  <div key={label}>
                    <div style={{ fontSize:11, fontWeight:700, color:C.navy, marginBottom:8 }}>{label}</div>
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                      <div>
                        <label style={{ fontSize:10, color:C.muted, display:'block', marginBottom:4 }}>SPH</label>
                        <select value={form[sphKey]} onChange={e=>set(sphKey,e.target.value)} style={SEL}>
                          {SPH_VALUES.map(v=><option key={v} value={v}>{v||'Select'}</option>)}
                        </select>
                      </div>
                      <div>
                        <label style={{ fontSize:10, color:C.muted, display:'block', marginBottom:4 }}>CYL</label>
                        <select value={form[cylKey]} onChange={e=>set(cylKey,e.target.value)} style={SEL}>
                          {CYL_VALUES.map(v=><option key={v} value={v}>{v||'None'}</option>)}
                        </select>
                      </div>
                    </div>
