...
const urgency = (days) => {
  if (days >= 60) return { color:'var(--danger)', bg:'#fee2e2', label:'Overdue',    dot:'var(--danger)' };
  ...
  return              { color:'var(--text-muted)',    bg:'var(--bg-sunken)',   label:'Recent',     dot:'#9ca3af' };
};
...
function CustomerRow({ customer, orders, onCall }) {
  ...
  return (
    <div style={{ background:'var(--bg-surface)', border:`1px solid var(--border)`, borderRadius:'var(--r-lg)', overflow:'hidden', marginBottom:10 }}>
      {/* Main row */}
      <div style={{ display:'flex', alignItems:'center', gap:14, padding:'14px 16px', cursor:'pointer' }}
        onClick={()=>setExpanded(e=>!e)}>

        {/* Avatar */}
        <div style={{ width:44, height:44, borderRadius:'50%', background:'var(--navy)', display:'flex', alignItems:'center', justifyContent:'center', color:'#e8c96a', fontSize:16, fontWeight:700, flexShrink:0 }}>
          {initials}
        </div>
        ...
        <div style={{ flexShrink:0, textAlign:'center' }}>
          <div style={{ background:urg.bg, color:urg.color, fontSize:11, fontWeight:700, padding:'3px 10px', borderRadius:'var(--r-full)', marginBottom:3 }}>
            {urg.label}
          </div>
          <div style={{ fontSize:10, color:'var(--text-muted)' }}>{days}d ago</div>
        </div>

        {/* Balance */}
        <div style={{ flexShrink:0, textAlign:'right' }}>
          <div style={{ fontFamily:"var(--font-display)", fontSize:18, fontWeight:700, color:'var(--danger)' }}>
            {fmt(totalBalance)}
          </div>
          <div style={{ fontSize:11, color:'var(--text-muted)' }}>{orders.length} order{orders.length!==1?'s':''}</div>
        </div>

        {/* Expand arrow */}
        <div style={{ fontSize:14, color:'var(--text-muted)', flexShrink:0 }}>{expanded?'▲':'▼'}</div>
      </div>
      ...
      <div style={{ padding:'0 16px 12px', display:'flex', gap:8, flexWrap:'wrap' }}>
        ...
        <a href={`tel:${customer.phone}`}
          style={{ padding:'9px 14px', background:'var(--bg-sunken)', color:'var(--navy)', border:`1.5px solid var(--border)`, borderRadius:9, fontSize:13, fontWeight:600, textDecoration:'none', display:'flex', alignItems:'center', gap:6 }}>
          📞 Call
        </a>
        <button onClick={()=>setExpanded(e=>!e)}
          style={{ padding:'9px 14px', background:'var(--bg-surface)', color:'var(--text-muted)', border:`1.5px solid var(--border)`, borderRadius:9, fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'inherit', marginLeft:'auto' }}>
      ...
      {expanded && (
        <div style={{ borderTop:`1px solid var(--border)` }}>
          {orders.map((order,i)=>{
            ...
            return (
              <div key={order.id} style={{ padding:'12px 16px', borderBottom:i<orders.length-1?`1px solid var(--bg-sunken)`:undefined, display:'flex', gap:12, alignItems:'flex-start' }}>
                {/* Order details */}
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:4, flexWrap:'wrap', gap:6 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                      <span style={{ fontSize:13, fontWeight:700, color:'var(--navy)' }}>{order.order_number}</span>
                      <span style={{ background:'#fee2e2', color:'var(--danger)', fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:20 }}>
                        Balance Due
                      </span>
                    </div>
                    <span style={{ fontFamily:"var(--font-display)", fontSize:16, fontWeight:700, color:'var(--danger)' }}>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(120px,1fr))', gap:6, marginBottom:8 }}>
                    {[
                      ...
                      <div key={f.l} style={{ background:'var(--bg-sunken)', borderRadius:7, padding:'6px 10px' }}>
                        <div style={{ fontSize:9, fontWeight:700, textTransform:'uppercase', letterSpacing:'.7px', color:'var(--text-muted)', marginBottom:2 }}>{f.l}</div>
                        <div style={{ fontSize:12, fontWeight:600, color:'var(--navy)' }}>{f.v}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <button onClick={()=>openWA(customer.phone, msg)}
                    style={{ padding:'7px 14px', background:'#25D366', color:'white', border:'none', borderRadius:'var(--r-sm)', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>
  ...
  return (
    <div style={{ fontFamily:"var(--font-body)" }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:4, flexWrap:'wrap', gap:10 }}>
        <div>
          <h1 style={{ fontFamily:"var(--font-display)", fontSize:24, color:'var(--navy)', margin:0 }}>💰 Balance Follow-up</h1>
          <p style={{ fontSize:13, color:'var(--text-muted)', margin:'4px 0 0' }}>Customers with unpaid balance — send WhatsApp reminders</p>
        </div>
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={load}
            style={{ padding:'9px 14px', background:'var(--bg-surface)', border:`1.5px solid var(--border)`, borderRadius:9, fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'inherit', color:'var(--text-muted)' }}>
            🔄 Refresh
          </button>
          {filtered.length > 0 && (
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))', gap:12, margin:'20px 0' }}>
        {[
          ...
          { l:'Customers',       v:data.length,      c:'var(--navy)'                  },
          { l:'Orders Due',      v:totalOrders,      c:'#2563eb'               },
          { l:'30+ Days',        v:overdue30,        c:'#b45309'               },
          { l:'60+ Days',        v:overdue60,        c:'var(--danger)'                },
        ].map(s=>(
          <div key={s.l} style={{ background:s.dark?'var(--navy)':'white', border:`1px solid ${s.dark?'var(--navy)':'var(--border)'}`, borderRadius:'var(--r-lg)', padding:'14px 16px', textAlign:'center' }}>
            <div style={{ fontSize:10, fontWeight:700, textTransform:'uppercase', letterSpacing:'.8px', color:s.dark?'var(--gold)':'var(--text-muted)', marginBottom:5 }}>{s.l}</div>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:16, flexWrap:'wrap' }}>
        <input value={search} onChange={e=>setSearch(e.target.value)}
          ...
          style={{ flex:1, minWidth:180, padding:'9px 14px', border:`1.5px solid var(--border)`, borderRadius:9, fontSize:13, fontFamily:'inherit', outline:'none', background:'var(--bg-surface)' }}/>
        ...
        <div style={{ display:'flex', gap:6 }}>
          {[
            ...
            <button key={f} onClick={()=>setFilter(f)}
              style={{ padding:'8px 14px', borderRadius:'var(--r-sm)', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'inherit', border:`1.5px solid ${filter===f?'var(--navy)':'var(--border)'}`, background:filter===f?'var(--navy)':'white', color:filter===f?'white':'var(--text-muted)' }}>
              {l}
            </button>
          ))}
        </div>
        ...
        <select value={sortBy} onChange={e=>setSortBy(e.target.value)}
          style={{ padding:'8px 12px', border:`1.5px solid var(--border)`, borderRadius:9, fontSize:13, fontFamily:'inherit', outline:'none', background:'var(--bg-sunken)', color:'var(--navy)', cursor:'pointer' }}>
      ...
      <div style={{ display:'flex', gap:14, marginBottom:14, fontSize:12, color:'var(--text-muted)', flexWrap:'wrap' }}>
        {[
          { dot:'var(--danger)', label:'60+ days overdue' },
      ...
      {loading
        ...
        : !filtered.length
          ? <div style={{ textAlign:'center', padding:40, color:'var(--text-muted)', background:'var(--bg-surface)', borderRadius:'var(--r-lg)', border:`1px solid var(--border)` }}>
